...
if __name__ == "__main__":
  ...
  if args.camera is not None:
    ...
    # Use raspistill to capture image
    try:
        subprocess.run([
            'raspistill',
            '-o', temp_file.name,
  ...
  # Initialize TFLite interpreter with optimizations
  interpreter_options = tf.lite.InterpreterOptions()
  
  # Try to enable XNNPACK delegate for better CPU performance
  try:
    xnnpack_delegate = load_delegate('libedgetpu.so.1')
    interpreter_options.add_delegate(xnnpack_delegate)
  except:
    print("XNNPACK delegate not available, continuing without it")

  interpreter = tflite.Interpreter(
      model_path=args.model_file,
      num_threads=args.num_threads,
      experimental_delegates=[interpreter_options])
  ...
  while True:
    if args.camera is None:
      img = Image.open(args.image).resize((input_width, input_height), Image.LANCZOS)
    else:
      try:
        # Capture image directly at the required size
        subprocess.run([
            'raspistill',
            '-o', temp_file.name,
            '-w', str(input_width),
        ...
        # Read the captured image without additional resizing
        img = Image.open(temp_file.name)
      except Exception as e:
        print(f"Error capturing image: {e}")
        break

    # Convert to RGB if needed (faster than BICUBIC resampling)
    if img.mode != 'RGB':
        img = img.convert('RGB')

    # Pre-allocate numpy array for input data
    input_data = np.zeros((1, input_height, input_width, 3), dtype=np.float32)
    
    # Convert image to numpy array and normalize in one step
    img_array = np.array(img, dtype=np.float32)
    if floating_model:
        input_data[0] = (img_array - args.input_mean) / args.input_std
    else:
        input_data[0] = img_array
