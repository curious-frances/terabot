...
class HardwareInterface:
    ...
    def get_imu(self):
        ...
        while True:
            ...
            try:
                decoded_data = msgpack.unpackb(data)
                print("Received IMU data:", decoded_data)  # Print received data for debugging
                if 'yaw' in decoded_data:
                    return decoded_data['yaw']
                else:
                    print("Warning: No 'yaw' key in IMU data. Available keys:", list(decoded_data.keys()))
                    return 0  # Return 0 if yaw not found
            except ValueError as e:
